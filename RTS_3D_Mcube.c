...
int main(){
    ...
    struct Vec3D resultx[8];   //--AFTER ROTATING ALONG X AXIS RESULT VECTOR IS STORED IN resultx--//
    struct Vec3D resulty[8];   //--AFTER ROTATING ALONG Y AXIS RESULT VECTOR IS STORED IN resulty--//
    struct Vec3D resultz[8];   //--AFTER ROTATING ALONG Z AXIS RESULT VECTOR IS STORED IN resultz--//
    struct Vec3D resultf[8];   //--AFTER ROTATING ALONG Z AXIS FINAL RESULT IS STORED IN resultf---//
...
//----------------------------------------RENDERING LOOP STARTS----------------------------------------------------------------------//
    ...
    while(1){
        ...
        for(int i=0;i<8;i++){
            matXvec3d( rotationX, cubeVertex[i], &resultx[i]);
            matXvec3d( rotationY,    resultx[i], &resulty[i]);
            matXvec3d( rotationZ,    resulty[i], &resultz[i]);
            matXvec3d(projection,    resultz[i], &resultf[i]);
        }
